...
type TransDataAccess(transDataList:TransferDataList,req,syncer) =
    ...
    member this.SetAndSync jobID data=d (TransferDataList.setAndSync transDataList syncer jobID) data
    member this.Get id=d (TransferDataList.get transDataList) id
    member this.Remove id=d (TransferDataList.remove transDataList) id
...
type DBAccess(jobDB:JobDataBase)=
    ...
    member this.TransDataAccess= TransferDataList.acessFuncs jobDB.TransferDataList jobDB.SyncEvents
    ...
    member this.CancelJob id=d (fun id-> (JobList.getJob jobDB.JobList id).CancelToken.Cancel()) id
    member this.AddJob sourceID makeJob transData=d (addJob jobDB sourceID makeJob) transData
