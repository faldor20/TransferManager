...
//https://gist.github.com/bobend/ae229860d4f69c563c3555e3ccfc190d
    ...
    let pipeStream source dest= 
        ...
        Tasks.Task.WhenAll(writing,reading).Wait()
    let simplewriter (source:Stream) (dest:Stream)=
        ...
        let mutable keepReading = true
        while keepReading do
            let read = source.Read(dataArray, 0, array_length)
            if 0 = read then
                //Finish reading
                keepReading<- false
            else
               // bwWrite.Write(dataArray, 0, read)
                dest.Write(dataArray, 0, read)
                //Continue reading
                keepReading <- true
    let ffmpegstream args dest destFile writer=
        ...
        writer proc.StandardOutput.BaseStream ftpWriter
        ftpWriter.Close() 
